...
async def duel(ctx, victim_str=None, *args):
	...
	if "private" not in args and "public" not in args:
		...
		client.spectate_msgs[duel_msg.id] = duel_channel

	await game_on(ctx, duel_channel, duelist, victim, duel_msg)
	del client.spectate_msgs[duel_msg.id]
...
async def on_raw_reaction_add(payload):
	...
	if payload.message_id in client.spectate_msgs.keys() and payload.emoji.name == "👁️" and payload.user_id != client.user.id:
		await client.spectate_msgs[payload.message_id].set_permissions(payload.member, read_messages=True, send_messages=False)
...
# 	if payload.message_id in client.spectate_msgs.keys() and payload.emoji.name == "👁️" and payload.user_id != client.user.id:
# 		await client.spectate_msgs[payload.message_id].set_permissions(message.auth, read_messages=False, send_messages=False)
